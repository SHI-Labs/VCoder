...
from vcoder_llava.train.vcoder_llava_trainer import VCoderLLaVATrainer
...
class LazySegSupervisedDataset(Dataset):
    """Dataset for supervised fine-tuning."""

    def __init__(self, tokenizer: transformers.PreTrainedTokenizer,
                 data_args: DataArguments):
        super(LazySegSupervisedDataset, self).__init__()
        
        list_data_dict = []

        for bucket in ["train", "unlabeled", "test"]:
            panoptic_labels = _obtain_seg_texts(os.path.join(data_args.seg_data_path, "coco_segm_text", bucket, "panoptic.txt"))
            semantic_labels = _obtain_seg_texts(os.path.join(data_args.seg_data_path, "coco_segm_text", bucket, "semantic.txt"))
            instance_labels = _obtain_seg_texts(os.path.join(data_args.seg_data_path, "coco_segm_text", bucket, "instance.txt"))
    ...
    def lengths(self):
        ...
        for sample in self.list_data_dict:
            ...
            length_list.append(sum(len(conv['value'].split()) for conv in sample['conversations']) + img_tokens + seg_tokens)
        return length_list
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            seg_folder = self.data_args.seg_data_path
            ...
            if self.data_args.image_aspect_ratio == 'pad':
                ...
                seg = expand2square(seg, tuple(int(x*255) for x in processor.image_mean))
                seg = seg_processor.preprocess(seg, return_tensors='pt')['pixel_values'][0]
            else:
                image = processor.preprocess(image, return_tensors='pt')['pixel_values'][0]
                seg = seg_processor.preprocess(seg, return_tensors='pt')['pixel_values'][0]
            sources = vcoder_preprocess_multimodal(
...
@dataclass
class DataCollatorForSegSupervisedDataset(object):
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    train_dataset = LazySegSupervisedDataset(tokenizer=tokenizer,
                                data_args=data_args)
    data_collator = DataCollatorForSegSupervisedDataset(tokenizer=tokenizer)
...
def vcoder_train():
    ...
    if model_args.seg_tune_adapter is not None:
        if 'mpt' in model_args.model_name_or_path:
            raise ValueError("MPT is not supported for Segmentation Adapted Training.")
        else:
            model = VCoderLlavaLlamaForCausalLM.from_pretrained(
    ...
    if training_args.bits in [4, 8]:
        ...
        for name, module in model.named_modules():
            ...
            if 'lm_head' in name or 'embed_tokens' in name or 'seg_embed_tokens' in name:
    ...
    data_module = make_supervised_data_module(tokenizer=tokenizer,
                                              data_args=data_args)
    trainer = VCoderLLaVATrainer(model=model,
                    tokenizer=tokenizer,
                    args=training_args,
                    **data_module)
