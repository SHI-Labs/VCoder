...
from vcoder_llava.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN
...
from vcoder_llava.mm_utils import tokenizer_image_token
...
def obtain_data_splits(data_args):
    def _get_labels(folder):
        return _obtain_seg_texts(os.path.join(data_args.image_folder, folder, "panoptic.txt"))
    ...
    labels_dict = {
        "coco": _get_labels("coco_segm_text/train/"),
        "gqa": _get_labels("gqa/seg_images/"),
        "vg/VG_100K": _get_labels("vg/vg/SEG_VG_100K/"),
        "vg/VG_100K_2": _get_labels("vg/vg/SEG_VG_100K_2/"),
        "ocr_vqa": _get_labels("ocr_vqa/seg_images"),
        "textvqa": _get_labels("textvqa/seg_images/"),
    }
...
def get_object_data_split(data_args):
    ...
    for bucket in ["train", "unlabeled", "test"]:
            panoptic_labels = _obtain_seg_texts(os.path.join(data_args.image_folder, "coco_segm_text", bucket, "panoptic.txt"))
            semantic_labels = _obtain_seg_texts(os.path.join(data_args.image_folder, "coco_segm_text", bucket, "semantic.txt"))
            instance_labels = _obtain_seg_texts(os.path.join(data_args.image_folder, "coco_segm_text", bucket, "instance.txt"))
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        list_data_dict = obtain_data_splits(data_args)
